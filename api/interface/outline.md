...
* the coarseness factor: the spatial resolution of the data to use. Source (native) spatial resolution is available for Source, Hour, Day, Month, and Year products. Coarsen-2 is available only for Month and Year products. User is required to specify the coarseness factor. The default is source resolution.
